...
function is_alfanum(str)
{
    return (/^[A-Za-z0-9]*$/.test(str));
}

function is_alfa(str)
{
    if (!str)
        return true;
...
function is_valid_arr(dataObject)
{
    let result = [];

    for (const object of dataObject)
    {
    const { id, username, email, age, location } = object;
    
        if (!is_alfanum(id) || !is_alfanum(username) || !email_valid(email) || !is_num(age) || !is_alfa(location))
            {
...
const res = is_valid_arr(dataObject);

function get_msg(res)
{
    const msg = [];
    res.forEach((item) => msg.push(item[0]));
    return msg.join('');
}
message = get_msg(res);
