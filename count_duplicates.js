...
//convert string to array using " " as a divider
const arr = string.split(" ");

//Function to count the repeated words in array, if not repited count is 1
//Returns string "<word><count><word><count>.."
function find_duplicate_in_array(array) {
    const count = {};  // Object to store the count of each unique item

    // Iterate through the input array
    array.forEach(item => {
        // If the item is already in the count object, increment its count
        //square bracket notation is necessary in certain situations, 
    ...
    // Iterate through the count object to build the result string
    for (let prop in count) {
        result += prop + count[prop];
    }

    //console.log(count);  // Log the count object to the console (for understanding)
    return result;  // Return the result string
}


const resultString = find_duplicate_in_array(arr);
