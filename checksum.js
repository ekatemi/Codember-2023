...
{
    [password, check] = item.split('-');
    return { password, check };
});

function no_duplicates(password)
{
    let str = [];
...
function checksum33(dataObject, pos)
{
    let result = [];
    pos = pos - 1;
    ...
    if (no_duplicates(password) === check)
        result.push(no_duplicates(password));  
    }
    return result[pos];
}


const arr = checksum33(dataObject, 33);
